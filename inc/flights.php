	...
	function flightsTable($airport, $type)
	{
		...
		foreach ($flights as $f)
		{
			...
			if ($type == "arrivals")
			{
				...
				$result .= '<td data-order="' . $f->arrivalTime . '" style="white-space: nowrap;">' . getHumanDateTime($f->arrivalTime) . '</td>';						
			}
			elseif ($type == "departures")
			{
				...
				$result .= '<td data-order="' . $f->departureTime . '" style="white-space: nowrap;">' . getHumanDateTime($f->departureTime) . '</td>';						
			}
			...
			if ($config["mode"] != 1){
				if ($f->booked == "prebooked"){
					...
					$result .= '<td data-order="1" data-search="prebooked"><button class="btn btn-warning btn-sm btn-block" onclick="return false;"><i class="fas fa-lock"></i> Available soon</button></td>';
				}
			}else{
				...
				if ($f->booked == "free") {
					if ($prebookMode) {
						$result .= '<td data-order="0" data-search="free"><button class="btn btn-success btn-sm btn-block" onclick="getFlight(' . $f->id . ')"><i class="fas fa-hand-paper"></i> Pre-book now!</button></td>';
					} else {
