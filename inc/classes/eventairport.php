...
class EventAirport
{
	...
	public static function GetAll($disabledsToo = false)
	{
		...
		if ($disabledsToo)
			...
			$sql = "SELECT * FROM airports WHERE enabled=1 ORDER BY `id`";
	...
	public function Update($array)
	{
		...
		{
			if ($db->GetSQL()->query("UPDATE airports SET icao='" . $array["icao"] . "', name='" . $array["name"] . "', `id`='" . $array["order"] . "', enabled=" . $array["enabled"] . " WHERE id=" . $this->id))
				return 0;
		}
		else
			return 403;
	...
	public static function Create($array)
	{
		...
		if (Session::LoggedIn() && Session::User()->permission > 1)
		{
			if ($db->GetSQL()->query("INSERT INTO airports (icao, name, `id`, enabled, type, latitude, longitude, elevation, iata, country) VALUES ('" . $array["icao"] . "', '" . $array["name"] . "', " . $array["order"] . ", ". $array["enabled"] . ", '". $array["tipo"] . "', ". $array["latitude"] . ", ". $array["longitude"] . ", ". $array["altitude"] . ", '". $array["iata"] . "', '". $array["country"] ."')"))
				return 0;
		}
		else
			return 403;
