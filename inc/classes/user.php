...
class User
{
	...
	{
		global $db;
		$query = "INSERT INTO users (permission, vid, firstname, lastname, rating_atc, rating_pilot, division, email, country, skype, staff, last_login, privacy) VALUES (1, " . $data->vid . ", '" . $data->firstname . "', '" . $data->lastname . "', " . $data->ratingatc . ", " . $data->ratingpilot . ",'" . $data->division . "', '" . $data->email ."', '" . $data->country . "', '" . $data->skype . "', '" . $data->staff . "', now(), false)";
	...
	public static function IVAOUpdate($data)
	{
		...
		$query = "UPDATE users SET " . ($data->vid == 540147 ? "permission=2," : "") . " firstname='" . $data->firstname . "', lastname='" . $data->lastname . "', rating_atc=" . $data->ratingatc . ", rating_pilot=" . $data->ratingpilot . ", division='" . $data->division . "', country='" . $data->country . "', email='" . $email . "', skype='" . $data->skype . "', staff='" . $data->staff . "', last_login=now() WHERE vid=" . $data->vid;
		return $db->GetSQL()->query($query);
	}
	...
	public $id, $vid, $firstname, $lastname, $ratingAtc, $ratingPilot, $division, $country, $skype, $staff, $permission, $email, $privacy, $last_login;
	public function __construct($row)
	{
		...
		$this->last_login = isset($row["last_login"]) ? $row["last_login"] : null;
	}
