...
class Config
{
	...
	public static function UpdateGeneral($array)
	{
		...
		{
			// Ensure prebook has a value (default to 'false' if not set)
			$prebook = isset($array["prebook"]) ? $array["prebook"] : "false";
			
			if (Config::Write("event_name", $array["event_name"]) == 0 &&
				Config::Write("mode", $array["mode"]) == 0 &&
				Config::Write("prebook", $prebook) == 0 &&
				...
				Config::Write("event_about", $array["event_about"]) == 0 &&
				Config::Write("event_banner", $array["event_banner"]) == 0 &&
