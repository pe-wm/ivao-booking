...
class Slot
{
	...
	/**
	 * Accepts a slot request (changes booked status from 1 to 2)
	 * Only admins can accept slots
	 * @return int error code: 0 = no error, 403 = forbidden, 1 = slot is not in requested state, -1 = other error
	 */
	public function Accept()
	{
		...
		if (Session::LoggedIn() && $sesUser->permission > 1)
		{
			if ($this->booked == "requested")
			{
				if ($db->GetSQL()->query("UPDATE slots SET booked=2 WHERE id=" . $this->id))
				{
					...
					if ($u && !empty($u->email))
					{
						...
						Email::Prepare($email, $u->firstname . " " . $u->lastname, $u->email, "Private slot accepted");
					}
					return 0;
				}
			}
			else
				return 1; // Not in requested state
		}
		else
			return 403;
		return -1;
