		...
		success: function(data) {
			...
			if (data.booked == "free")
			{
				...
				$("#btnFltBook").click(function() {
					...
					if (isPrebook) {
						...
						$("#btnFltConfirm").text("I agree, pre-book the flight");
					} else {
						$("#fltInfobox").attr("class", "alert alert-warning")
			...
			if (data.sessionUser && ((data.bookedByUser && data.bookedByUser.vid == data.sessionUser.vid) || (data.sessionUser.permission >= 2)))
			{
				...
				$("#fltTafOrigin").html("TAF " + data.originIcao)
					.attr("onclick", "getWx('#fltWxResult', 'json', { type: 'wx', icao: '" + data.originIcao + "', taf: true })");
				$("#fltMetarDestination").html("METAR " + data.destinationIcao)
					.attr("onclick", "getWx('#fltWxResult', 'json', { type: 'wx', icao: '" + data.destinationIcao + "' })");
				$("#fltTafDestination").html("TAF " + data.destinationIcao)
					.attr("onclick", "getWx('#fltWxResult', 'json', { type: 'wx', icao: '" + data.destinationIcao + "', taf: true })");
			...
			if (data && data.error == 0)
			{
				if (isPrebook) {
					swal2({
						...
						text: "You will receive a confirmation email. Please confirm your booking once you are sure you can fly it.",
						type: "success",
					...
					swal2({
						...
						text: "Don't forget to be there on time!",
						type: "success",
