	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.sessionUser && data.sessionUser.permission > 1)
			{
				...
				// Show Accept/Reject buttons only if slot is requested
				if (data.booked == "requested") {
					$("#btnSlotAdminAccept").show().attr("onclick", "acceptSlot(" + data.id + ")");
					$("#btnSlotAdminReject").show().attr("onclick", "rejectSlot(" + data.id + ")");
				} else {
					$("#btnSlotAdminAccept").hide();
					$("#btnSlotAdminReject").hide();
				}
...
function acceptSlot(id) {
	swal2({
		title: "Accept this slot request?",
		text: "The slot will be granted to the user.",
		type: "question",
		showCancelButton: true,
		...
		if (result.value) {
			$.ajax({
				...
				type: "POST",
				url: "json",
				data: { "type": "slots", "action": "accept", "id": id },
				success: function(data) {
					if (data && data.error == 0) {
						toast({
							title: "Slot has been accepted!",
							type: "success"
						});
						getSlot(id);
						
						// Refresh timeframe if available
						if (typeof aGetTimeframe === "function") {
							var timeframeId = $("#slotId").closest("[data-timeframe-id]").data("timeframe-id");
							if (timeframeId) aGetTimeframe(timeframeId);
							aGetTimeframes();
						} else {
							pageToBeReloaded = true;
						}
...
function rejectSlot(id) {
	swal2({
		title: "Reject this slot request?",
		text: "The slot will be deleted permanently.",
		type: "warning",
		showCancelButton: true,
		...
		if (result.value) {
			$.ajax({
				...
				type: "POST",
				url: "json",
				data: { "type": "slots", "action": "delete", "id": id },
				success: function(data) {
					if (data && data.error == 0) {
						toast({
							title: "Slot has been rejected and deleted!",
							type: "success"
						});
						$("#slot").modal("hide");
